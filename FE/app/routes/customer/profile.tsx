        ...
        <div className="md:col-span-2 p-6 flex flex-col justify-center bg-white border border-slate-200/70 rounded-xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="flex flex-col py-2 border-b border-gray-100/70 space-y-2">
...
export default function CustomerProfilePage() {
  ...
  const handleAvatarSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Compress the image before uploading to dramatically speed up the API request
      ...
      toast.error(normalizeError(err).message || "Lỗi cập nhật ảnh đại diện");
    } finally {
