...
builder.Services.AddInfrastructure(builder.Configuration,
    migrationsAssembly: "LG.Core.Infrastructure");
...
builder.Services.AddSwaggerGen(opt =>
{
    ...
    // Hỗ trợ IFormFile upload trong Swagger UI
    opt.MapType<IFormFile>(() => new Microsoft.OpenApi.Models.OpenApiSchema
    {
        Type = "string",
        Format = "binary"
    });
...
builder.Services.AddHealthChecks()
    .AddDbContextCheck<CoreDbContext>("postgres");
...
// Global exception handler — bỏ qua Swagger/Health paths để không nuốt lỗi của chúng
app.UseWhen(
    ctx => !ctx.Request.Path.StartsWithSegments("/swagger")
         && !ctx.Request.Path.StartsWithSegments("/health"),
    appBranch => appBranch.UseMiddleware<ExceptionHandlingMiddleware>()
);

app.UseSwagger();
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "MuaHo Core API v1");
